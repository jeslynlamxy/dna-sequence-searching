...
def test_string(query, genome_sequence, algo):
    ...
    if algo == "bf":
        ...
        algo_obj = rabin_karp.RabinKarp(query)
    elif algo == "kmp":
        algo_obj = knuth_morris.KnuthMorris(query)
    elif algo == "sbf":
        algo_obj = semi_brute_force.SemiBruteForce(query)
    else:
    ...
    for fna in fna_list:
        ...
        if genome_sequence:
            ...
            for algo in algo_list:
                # test on all algos and create csv files
...
def test_fixed_query():
    ...
    csv_columns = ["Genome Size", "BF Time", "BM Time", "RK Time", "KMP Time", "SBF Time"]
    csv_data = []
    bf = brute_force.BruteForce(query_string)
    bm = boyer_moore.BoyerMoore(query_string)
    rk = rabin_karp.RabinKarp(query_string)
    kmp = knuth_morris.KnuthMorris(query_string)
    sbf = semi_brute_force.SemiBruteForce(query_string)
    x = 10000
    ...
    for value in range(10, x):
        ...
        # rabin karp
        start = time.perf_counter()
        rk.search(genome_string)
        stop = time.perf_counter()
        rk_time_taken = round(stop - start, 5)
        ...
        # semi brute force
        start = time.perf_counter()
        sbf.search(genome_string)
        stop = time.perf_counter()
        sbf_time_taken = round(stop - start, 5)

        # append data
        csv_data.append([value, bf_time_taken, bm_time_taken, rk_time_taken, kmp_time_taken, sbf_time_taken])
    csv_dataframe = pd.DataFrame(csv_data, columns=csv_columns)
...
def test_fixed_genome():
    ...
    csv_columns = ["Query Size", "BF Time", "BM Time", "RK Time", "KMP Time", "SBF Time"]
    csv_data = []
    x = 1000
